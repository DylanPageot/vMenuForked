    ...
    public static class CommonFunctions
    {
        ...
        public static async Task TeleportToPlayer(IPlayer player, bool inVehicle = false)
        {
            ...
            if (player.IsActive || player is InfinityPlayer)
            {
                ...
                await TeleportToCoords(playerPos,false,true);
        ...
        /// <returns></returns>
        public static async Task TeleportToCoords(Vector3 pos, bool safeModeDisabled = false, bool player = false)
        {
            Vector3 coordsA = new (Vector2.Empty, 0.0f), coordsB = new(Vector2.Empty,0.0f);
            if (!player)
            {
                ShowNotification("Vous allez bientôt être téléporté. Veuillez patienter quelques secondes sans bouger...");
                coordsA = GetEntityCoords(Game.PlayerPed.GetHashCode(),false);
                await Delay(10000);
                coordsB = GetEntityCoords(Game.PlayerPed.GetHashCode(), false);
            }
            if (player || GetDistanceBetweenCoords(coordsA.X, coordsA.Y, coordsA.Z, coordsB.X, coordsB.Y, coordsB.Z,false) < 2.0f)
            {
                if (!safeModeDisabled)
                {
            ...
            float heading = -1f)
        {
                ...
                GetVehicleClassFromName(vehicleHash) != 21)
            {
                ...
                TriggerEvent("LegacyFuel:SetFuelToMaxLevel",car.GetHashCode());
                SetVehicleFuelLevel(car.GetHashCode(),150.0f);
